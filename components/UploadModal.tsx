...
export const UploadModal: React.FC<UploadModalProps> = ({ isOpen, onClose, onComplete, projects = [], initialProjectId }) => {
  ...
  // --- Helper: Get Video Metadata & Frame ---
  ...
  const analyzeImageWithLocalModel = async (base64Image: string): Promise<{ hasHuman: boolean }> => {
    if (!detectorRef.current) {
      // Try loading again if null (fallback safety)
      if (window.cocoSsd) {
        detectorRef.current = await window.cocoSsd.load();
      } else {
        console.warn("COCO-SSD library not available");
  ...
  const processFiles = async () => {
    ...
    for (let i = 0; i < selectedFiles.length; i++) {
      ...
      try {
        ...
        // --- Critical Change: Use Aspect Ratio Classes ---
        ...
        newVideos.push({
          ...
          url: fileUrl, // Keep temporary URL for immediate playback
          thumbnail: metadata.thumbnailBase64,
          projectId: finalProjectId // Assign the selected project ID
        });

        addLog(`识别完成: [${typeStr}] [${humanStr}] -> 归档至 [${targetProjectName}]`, 'done');

      } catch (err: any) {
        let errorMsg = `处理失败: ${file.name}`;
        if (err.name === 'QuotaExceededError') {
           errorMsg = `储存空间不足，保存失败: ${file.name}`;
           addLog(`错误: 浏览器存储空间已满`, 'loading');
        }
